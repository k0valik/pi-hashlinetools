...
describe("resolveToCwd", () => {
  ...
  it("resolves a relative path against cwd", () => {
    expect(resolveToCwd("src/main.ts", cwd)).toBe(resolve(cwd, "src/main.ts"));
  });
  ...
  it("expands ~ to home directory", () => {
    expect(resolveToCwd("~/file.txt", cwd)).toBe(`${os.homedir()}/file.txt`);
  });
  ...
  it("does not treat @~ as home-directory expansion", () => {
    expect(resolveToCwd("@~/notes.md", cwd)).toBe(resolve(cwd, "@~/notes.md"));
  });
});
