...
describe("writeFileAtomically", () => {
  ...
  it.skipIf(process.platform === "win32")(
    "preserves the target file mode when replacing an existing file",
    async () => {
  ...
  it.skipIf(process.platform === "win32")(
    "updates a symlink target without replacing the symlink",
    async () => {
      await withTempFile(
        "target.txt",
        "before\n",
  ...
  it.skipIf(process.platform === "win32")(
    "follows a dangling symlink chain through to the missing terminal target",
    async () => {
      await withTempFile("seed.txt", "seed\n", async ({ cwd }) => {
