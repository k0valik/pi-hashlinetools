...
export async function loadFileKindAndText(
  ...
  try {
    ...
    //
    // `ignoreBOM: true` keeps the BOM in the decoded string (as a leading
    // U+FEFF) instead of silently consuming it. This is required for the
    // BOM-preservation contract: when a file has a UTF-8 BOM, we re-emit it
    // on write. Without this option, `TextDecoder("utf-8")` strips the BOM
    // per the WHATWG encoding spec, the file-text would never start with
    // \uFEFF, and `stripBom` would return `bom: ""` — so the BOM would be
    // lost on every edit to the first line.
